...
public class UserDatastore {
	...
	public static boolean add(UserEntry user) {
		...
		try {
			...
			writeChannel.write(java.nio.ByteBuffer.wrap((user.getProfilePhoto())));
			writeChannel.closeFinally();
			
			// your blobKey to your data in Google App Engine BlobStore
			BlobKey blobKey = fileService.getBlobKey(file);
			entity.setProperty(Globals.KEY_USER_BLOB_KEY, blobKey.getKeyString());
		} catch (IOException e) {
	...
	private static UserEntry getUserEntryFromEntity(Entity entity){
		...
		user.setGender(Integer.parseInt(String.valueOf((Long)(entity.getProperty(Globals.KEY_USER_GENDER)))));
		user.setClassYear(Integer.parseInt(String.valueOf((Long) entity.getProperty(Globals.KEY_USER_CLASS_YEAR))));
